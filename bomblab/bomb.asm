...
0000000000400ee0 <phase_1>:
  ...
  ; esi 是传入 <strings_not_equal> 的第二个参数
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi 
  ;第一个参数是 edi ,这里没有改变，也就是 main 传进来的第一个参数
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal> 
  ; test 用来测试 eax 是正数，负数还是零
  ; eax 是 <strings_not_equal> 的返回值
  400eee:	85 c0                	test   %eax,%eax 
  ; 0 就跳转，也就是我们的输入值需要和 $0x402400 相等
...
0000000000400efc <phase_2>:
  ...
  ; 读 6 个数字进来，很显然返回的指针就是 rsp 这个地方，数字存在这里
  400f05:	e8 52 05 00 00       	callq  40145c <read_six_numbers>
  ; 比较第一个数和 0x1 ,肯定第一个数字就是 0x1 ，否则就爆了
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp)
  400f0e:	74 20                	je     400f30 <phase_2+0x34>
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  ; 第二个数减 4 就是第一个数就是 1 放进 eax
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax
  ; 加倍 eax 变成 2
  400f1a:	01 c0                	add    %eax,%eax
  ; 将 2 和第二个数比较，那么第二个数就是 2
  400f1c:	39 03                	cmp    %eax,(%rbx)
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  400f20:	e8 15 05 00 00       	callq  40143a <explode_bomb>
  ; rbx + 4 也就是后一个数，当 rbx 是第一个数那么加 4 就是第二个数
  400f25:	48 83 c3 04          	add    $0x4,%rbx
  ; rbp（第七个数） 和 rbx 比较，rbx 不能超过 7 ，循环的终止
...
0000000000400f43 <phase_3>:
  ...
  ; 第 2 个参数是 $0x4025cf ,用 gdb 调试出来是 "%d %d"
  ...
  ; 因此我们可以猜测 sscanf 函数的返回值肯定是 1 ，第一个参数不知道，第二个是 "%d %d", 第三第四就是我们需要的值
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb>
  ; 比较 0x7 和第三个参数的大小 
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)
  ; 超过跳转，那么意味着我们输入的值要小于等于 7
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>
  ; 把第一个值写入 eax
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax
  ; 直接跳转，跳转到 0x402470 + %rax * 8 的地方，当第一个参数是 0 时，我们用 gdb 看到这个位置是0x00400f7c
...
000000000040100c <phase_4>:
  ...
  ; 比较 0xe 和 x ，小于等于跳转，我们的 x 需要小于等于 0xe
  ...
  ; 第一个参数 就是我们输入的第一个数，第二个参数是 0x0，第三个参数是 0xe
  401048:	e8 81 ff ff ff       	callq  400fce <func4>
  ; 分析后我们卡一波bug 让 x = 7 ，就不用总是跳转了
  ; 返回值 eax = 0
